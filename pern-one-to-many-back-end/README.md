...
- If you have students in a class, but the class is canceled, you don't want to delete the students from the database.
- In contrast, if you have **many** posts for **one** product, if the product is deleted from the database, you would likely want to delete all the posts that went with it because the posts don't make sense without the product.

For now, we are just building simple apps with the goal of learning rather than making a robust and compelling user experience. We also do not yet have to consider what happens if we have thousands or millions of users. We're going to focus on syntax and code examples that are simple, so you can have a solid foundation to build on so that when you encounter new challenges, you can build on your foundation.
...
- When setting up the foreign key, there is a constraint keyword `REFERENCES`. This means that posts cannot be created without a valid user `PRIMARY KEY` that exists in the user table.
- If the user table gets deleted, the posts table should get deleted as well, therefore you can add `ON DELETE CASCADE`, which will remove the posts table if the user table gets deleted.
...
Your SQL query would could like this:

```sql
 "SELECT * FROM posts WHERE user_id=$1",
```

Let's imagine that the user view is just account information. Would you always need to load the posts? Probably not. Most likely you would want a separate view where you can see the user's post.
...
Would you need to create routes that were in the format `posts/:post_id/users`? This resource would be described as _show one post and include the all the users_. That doesn't make much sense. What about `posts/:post_id/users/:user_id`. This route makes more sense. When creating nested resources it is not necessary to create every combination. You must design your routes to make sense. You should rely on user stories and other planning tools to guide your routes.
...
As you can see, the route here is `/posts/`, but the request has access to the parameters from the users route.

## Reference build

This build is the back-end for bookmarks, where bookmarks can have many posts.
